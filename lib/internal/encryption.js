...
export default class Encryption {
  ...
  async encrypt (message) {
    ...
    const initializationVector = await window.crypto.getRandomValues(
      new Uint8Array(12)
    )
    ...
    const result = base64.encode('1' + tag + initializationVector + encryptedMessage)
  ...
  async decrypt (message) {
    const binaryMessage = base64.decode(message)
    const key = this.encryptionKey
