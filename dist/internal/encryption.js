...
export default class Encryption {
    ...
    async init(encryptionPassword, userIden) {
        ...
        const keyMaterial = await globalThis.crypto.subtle.importKey('raw', textEncoder.encode(encryptionPassword), {
            name: 'PBKDF2'
    ...
    async encrypt(message) {
        ...
        const initializationVector = globalThis.crypto.getRandomValues(new Uint8Array(12));
        const cipher = await globalThis.crypto.subtle.encrypt({ name: 'AES-GCM', iv: initializationVector }, key, new TextEncoder().encode(message));
    ...
    async decrypt(message) {
        ...
        const decrypted = await globalThis.crypto.subtle.decrypt({ name: 'AES-GCM', iv: initializationVector, tagLength: 128 }, key, mergedTagCipher);
