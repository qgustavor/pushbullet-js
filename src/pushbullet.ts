import Encryption from './internal/encryption'
import Stream from './internal/stream'
import type { RequestOptions } from './types'

import './internal/channels'
import './internal/chats'
import './internal/devices'
import './internal/ephemerals'
import './internal/pushes'
import './internal/subscriptions'
import './internal/texts'
import './internal/users'

/**
 * PushBullet API abstraction module.
 */
class PushBullet {
  static readonly API_BASE = 'https://api.pushbullet.com/v2'
  static readonly CHANNEL_INFO_END_POINT = PushBullet.API_BASE + '/channel-info'
  static readonly CHANNELS_END_POINT = PushBullet.API_BASE + '/channels'
  static readonly CHATS_END_POINT = PushBullet.API_BASE + '/chats'
  static readonly DEVICES_END_POINT = PushBullet.API_BASE + '/devices'
  static readonly EPHEMERALS_END_POINT = PushBullet.API_BASE + '/ephemerals'
  static readonly PUSH_END_POINT = PushBullet.API_BASE + '/pushes'
  static readonly SUBS_END_POINT = PushBullet.API_BASE + '/subscriptions'
  static readonly TEXTS_END_POINT = PushBullet.API_BASE + '/texts'
  static readonly UPLOAD_END_POINT = PushBullet.API_BASE + '/upload-request'
  static readonly USERS_END_POINT = PushBullet.API_BASE + '/users'

  apiKey: string
  encryption?: Encryption

  constructor (apiKey: string) {
    if (!apiKey) {
      throw new Error('API Key is required')
    }
    this.apiKey = apiKey
  }

  /**
   * Enables End-to-End encryption.
   */
  async enableEncryption (encryptionPassword: string, userIden: string): Promise<void> {
    this.encryption = new Encryption()
    return this.encryption.init(encryptionPassword, userIden)
  }

  /**
   * Return a new stream listener.
   */
  stream (): Stream {
    return new Stream(this.apiKey, this.encryption)
  }

  /**
   * Performs a GET request to an end point.
   */
  getList (endPoint: string, options: Record<string, any> = {}): Promise<Response> {
    const parameters: RequestOptions = {}
    const optionKeys = Object.keys(options)

    if (optionKeys.length > 0) {
      parameters.qs = {}
      optionKeys.forEach((key) => {
        parameters.qs![key] = options[key]
      })
    }

    return this.makeRequest('get', endPoint, parameters)
  }

  /**
   * Makes the request to the PushBullet API.
   */
  makeRequest (verb: string, endPoint: string, options?: RequestOptions | null): Promise<Response> {
    const fetchInit: RequestInit = {
      method: verb,
      headers: {
        'Access-Token': this.apiKey
      }
    }

    if (options?.qs) {
      const url = new URL(endPoint)
      for (const key of Object.keys(options.qs)) {
        url.searchParams.append(key, options.qs[key])
      }
      endPoint = url.toString()
    }

    if (options?.json) {
      fetchInit.body = JSON.stringify(options.json);
      (fetchInit.headers as Record<string, string>)['Content-Type'] = 'application/json'
    }

    return fetch(endPoint, fetchInit)
  }
}

export default PushBullet
export { PushBullet }
